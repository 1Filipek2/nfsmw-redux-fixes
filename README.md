...
- `NextGenGraphics.MostWanted.asi`: the worker loop sleeps 1 ms before asking for work again
...
patched  NextGenGraphics.MostWanted.asi (worker thread sleeps 1 ms when idle)
patched  MW360Tweaks.asi (no more per-frame loading of missing png textures)
```

Running it again prints `skip ... (already patched)`. A file with a different hash is left alone with `skip ... (unknown version, not touched)`.
...
- Frame times captured with PresentMon, 2 min per run
